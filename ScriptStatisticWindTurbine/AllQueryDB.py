class AllQueryDB:
    ...
    def query_to_different_period(id_turbine:int):
        return f'SELECT * FROM maintenance_turbine WHERE id_turbine={id_turbine}'

    @staticmethod
    def query_to_warning_and_error(id_turbine:int,date_init:str,date_finish:str):
        return f"SELECT * FROM value_sensor_error WHERE id_turbine={id_turbine} AND date>='{date_init}' AND date<='{date_finish}'"
