...
namespace ClientPltTurbine.Model.MaintenanceModel.Implementation
{
    public class MaintenanceModel :BaseModel, IMaintenanceModel
    {
        ...
        public Task SaveMaintenanceTurbines(Dictionary<string, (int id, string date, string datef, bool isNormalMaintenance)> infoTurbineMaintenance) => Task.Run(() =>
        {
            ...
            listTurbineMaintenance.ForEach((index,values)=>{
                var request = new MaintenanceTurbinesRequest() { Msg1 = new SaveTurbineInfoMaintenance() 
                { 
                    IdTurbine = values.id, Date = values.date, Datef = values.datef, IsNormalMaintenance=values.isNormalMaintenance
                },
                    IsFinish=total==(index+1)
                };
