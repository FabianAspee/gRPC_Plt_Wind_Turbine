...
namespace ClientPltTurbine.Model.LoadFile.Implementation
{
    public class LoadFileModel :BaseModel, ILoadFileModel
    {
        ...
        private Task ProcessFileBasic(DataTable file, string name, string type, string sep, int id)
        {
        ...
        private Task ProcessSensorFile(DataTable file, string name, string type, string sep, bool isEvent)
        {
            return Task.Run(async () =>
            {
                Console.WriteLine(name);
        ...
        private FileUploadRequestInfo ConstructTransferFile(string name, string type, string sep, JObject rowAndColumn) =>
        new()
        {
